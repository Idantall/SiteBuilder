 ...
 */
export default function HeroCardPinpoint({ highlight = "middle" }) {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const nums = [
        ...
        output.x,
        output.y,
        ...branches.flatMap((p: any) => [p.x, p.y]),
      ];
  ...
  return (
    <div className="w-full bg-gradient-to-b from-slate-50 to-white flex items-center justify-center p-4 md:p-6">
      <div className="w-full max-w-6xl rounded-3xl border border-slate-200 bg-white shadow-xl overflow-visible">
        {/* Header */}
        <div className="flex items-center justify-between px-4 md:px-5 py-4 border-b bg-slate-50/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 hidden sm:block">Live view of your delivery path</p>
            </div>
          </div>
        ...
        <div ref={containerRef} className="relative h-96 md:h-80">
          ...
          <div
            ref={sourceRef}
            className="absolute top-16 md:top-24 left-2 md:left-4 lg:left-6 w-48 md:w-56 rounded-xl bg-white/90 backdrop-blur border border-slate-200 shadow-md p-3 z-10"
          ...
          <BranchBox
            ref={topRef}
            hot={isTop}
            yClass="top-6 md:top-10"
            xPos="left-[40%] md:left-[45%] lg:left-[47%]"
            width="w-52 md:w-60"
          ...
          <BranchBox
            ...
            hot={false}
            yClass="top-20 md:top-28"
            xPos="right-2 md:right-6 lg:right-10"
            width="w-56 md:w-64"
          ...
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-white/80 backdrop-blur rounded-xl border border-slate-200 shadow-sm px-3 py-2">
            <ul className="flex items-center gap-2 md:gap-4 text-[11px] text-slate-600">
...
interface BranchBoxProps {
  ...
  width?: string;
  xPos?: string;
  iconHot: React.ReactNode;
  iconCold: React.ReactNode;
  title: string;
  subtitle?: string;
  resolved?: boolean;
  resolvedAccent?: boolean;
  style?: React.CSSProperties;
}

const BranchBox = forwardRef<HTMLDivElement, BranchBoxProps>(function BranchBox(
  ...
  const state = hot
    ? "border-rose-200 ring-4 md:ring-8 ring-rose-200/60"
    : resolved && (title?.includes("AI") || resolvedAccent)
      ? "border-emerald-300 ring-4 md:ring-8 ring-emerald-300/50 animate-pulse"
      : "border-slate-200";
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-xs md:text-sm font-medium text-slate-800">{title || (hot ? "Bottleneck" : "Healthy stage")}</p>
        {subtitle && <p className="text-[10px] md:text-xs text-slate-500">{subtitle}</p>}
      </motion.div>
    </motion.div>
  );
});

function SvgFlowDynamic({ anchors, hotIndex, resolved }: { anchors: any; hotIndex: number; resolved: boolean }) {
  ...
  const allNums = [source.x, source.y, midRight.x, midRight.y, output.x, output.y, ...branches.flatMap((p: any) => [p.x, p.y])];
  if (!allNums.every((n) => typeof n === "number" && Number.isFinite(n))) return null;

  // Compute viewBox extents (include output)
  const endX = (b: any) => b.x; // arrow tips land exactly on the card edge
  const maxEndX = Math.max(...branches.map((b: any) => endX(b)), output.x);
  const yAll = [source.y, ...branches.map((b: any) => b.y), output.y];
  ...
  // Build branch paths
  const branchPaths = branches.map((b: any, i: number) => {
